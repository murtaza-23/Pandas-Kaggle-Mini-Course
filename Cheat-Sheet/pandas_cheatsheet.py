...
# we can rename columns and set index to a specific column too
df = df.set_index(['column_name'])
df.rename(columns={'column_name': 'new_col_name'})
df.rename(index={0: 'index_name', 1: "index_2_name"})

df.rename_axis("index_row_name", axis='rows').rename_axis("index_col_name", axis='columns') # this is used to rename index for rows and columns

pd.concat([])   # this is used to concatenate to DataFrames, best when two DataFrames are indentical in terms of columns and their names

left_df = {}
right_df = {}

pd.merge(left_df, right_df, on='column_name', how='inner')  # this is used to merge two DataFrames, by default the join type is 'inner' and on= is used to merge on a common column name
# similarly we can use .join() which have the same syntax and arguments, but its join type is 'left' by default
# we can also use lsuffix='' and rsuffix='' to differentiate the column names of left and right DataFrames
